...
fun main(args: Array<String>) {
    ...
    keyEntries.forEach { entry ->
        if (entry[0].toString() != "Q#" && entry[1] != "original" && entry[2] != "generated") {
            renameMap[entry[0].toInt()] = listOf(entry[1].toString(), entry[2].toString())
        }
    }
    ...
    eachResults.withIndex().forEach { (index1, result) ->
        ...
        result.subList(1, 21).withIndex().forEach {(index2, eachResult) ->
            ...
            if (renameMap[index2 + 1]!!.indexOf(eachResult) == 0){
                ...
                // == 0 is original name, == 1 is generated name
            }
