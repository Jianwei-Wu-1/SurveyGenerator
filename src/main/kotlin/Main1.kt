...
fun main(args: Array<String>) {
    ...
    val input = "src/input/Input.txt"

    val gson = Gson()

    val renames: List<Rename> = File(input).bufferedReader().use { reader ->
        gson.fromJson(reader, Array<Rename>::class.java).toList()
    }

    val subRenames = renames.shuffled().subList(0, 20)

    val questionTests: List<QuestionTest> = subRenames.mapIndexed { index, rename ->
        val (oldName, newName, body) = rename

        val code: String = Formatter().formatSource("public class testClass { public void XXX() $body }")
    ...
    PrintWriter(File("/Users/wujianwei/ideaProjects/survey-generator/answer-key.csv").bufferedWriter()).use { writer ->
        ...
        subRenames.withIndex().forEach { (index, rename) ->
  ...
  "SurveyElements": [
    ...
    print(
            ...
            {
                "Type": "Question",
                "QuestionID": "QID${questionTests.size + 1}"
            }"""
    )
    ...
    },
    {
      ...
      "SecondaryAttribute": "${questionTests.size + 1}",
      "TertiaryAttribute": null,
      "Payload": null
    },
    {
